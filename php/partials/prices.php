<h2>Tarifs</h2>
<p class="item-intro text-muted">Vous trouverez-ici nos tarifs location tout public.
    Contactez-nous pour un devis personnalisé.
</p>

<img class="img-responsive img-centered" src="/assets/img/cochon.jpg" alt="Tirelire en cochon">

<div class="row">
    <p>
        <strong>NOS DIFFERENTES CATEGORIES DE KARTS : </strong>
        <br>
        <strong>Mini 160cc: </strong>
        <br>
        Le « mini kart » 160cc est une réplique exacte du karting adulte à une échelle inférieure. Cela permet
        aux enfants dès l’âge de 7 ans jusqu'à 13/14 ans de faire comme les grands (une taille minimum d'1m30 pour
        le conduire est obligatoire).
        <br><br>
        <strong>Elite : </strong>
        <br>
        Le karting dit « adulte » 270cc accessible dès l’âge de 13/14 ans, est un karting de toute dernière génération,
        qui va vous laisser rêver et vous permettre de vous comparer aux grands noms du sport automobile !
        <br><br>
        <strong>Bi-place : </strong>
        <br>
        permet à un adulte de piloter le karting avec à ses côtés, un enfant (minimum 5 ans ET 1m10) ou une autre personne.
        <br><br>
        <strong>Handikart :</strong>
        <br>
        avec ses commandes au volant, il permet le pilotage du karting pour certains types de handicaps.
    </p>
</div>

<div class="row">
    <p>
        <strong>LOC'KARTING VOUS ACCOMPAGNE DU DÉBUT À LA FIN : </strong>
        <br>
        Nos séances de location durent <strong>10 minutes</strong> et sont encadrées par un directeur/trice de course loisir.
        <br>
        Un équipier procédera à un briefing de sécurité avant de vous donner charlottes, casques et minerves pour
        les plus jeunes.
        <br>
        Un système de chronométrage informatisé vous permet de visualiser vos performances sur écran plat.
        <br>
        Et vous pouvez également <a href="https://lockarting.fr/application/" target="_blank">vous inscrire gratuitement
            sur notre application</a> pour récupérer vos chronos sur votre téléphone!
        (un email est nécessaire pour s'inscrire. Loc'karting ne revend pas vos données personnelles à des tiers.)
    </p>
</div>

<div class="row">
    <div class="table-responsive">
        <table class="table table-bordered table-condensed">
            <thead>
                <tr>
                    <th class="active">Séries de 10 minutes</th>

                    <th class="active">Mini Kart 160cc</th>

                    <th class="active">Kart Elite 270cc</th>

                    <th class="active">Kart Bi-place</th>

                    <!--<th class="active">Handikart</th>-->
                </tr>
            </thead>

            <tbody>
                <tr>
                    <td>1 série de 10 minutes</td>

                    <td>17,00 &#8364;</td>

                    <td>21,00 &#8364;</td>

                    <td>28,00 &#8364;</td>

                    <!--<td>22,00 &#8364;</td>-->
                </tr>

                <tr>
                    <td>2 s&#233;ries de 10 minutes</td>

                    <td>31,00 &#8364;</td>

                    <td>39,00 &#8364;</td>

                    <td>-</td><!--<td>54,00 &#8364;</td>-->

                    <!--<td>-</td>-->
                </tr>

                <tr>
                    <td>12 s&#233;ries de 10 minutes</td>

                    <td>165,00 &#8364;</td>

                    <td>210,00 &#8364;</td>

                    <td>-</td>

                    <!--<td>-</td>-->
                </tr>
            </tbody>
        </table>
    </div>
</div>


<div class="row">
    <p>
        <strong>Pour les petits veinards, nous avons une FORMULE ANNIVERSAIRE :</strong>
        <br>
        nous prenons les réservations par téléphone ou mail A PARTIR DE 6 PARTICIPANTS minimum.
        <br>
        La formule anniversaire comprend bien sûr du karting! : 2 sessions de 10 minutes de karting 160CC ainsi qu'un
        gâteau, des bonbons, des boissons, la remise des diplômes de pilote de karting pour chaque enfant sur le podium
        et une médaille pour la star de la journée! 33€/enfant.
    </p>
    <br>
    <p>Pour une demande chiffrée tel que : CE, accueil d'entreprise, EDV de jeune fille ou garçon,
        <a href="https://www.lockarting.fr/#contact"
            target="_blank">
            veuillez-nous faire une demande sur notre formulaire de contact.
        </a>
    </p>
</div>


<div class="row">
    <div class="table-responsive">
        <table class="table table-bordered table-condensed">
            <thead>
                <tr>
                    <th class="active" rowspan="2">Formules GP et<br>anniversaire</th>
                    <th class="active" colspan="2">GP 1</th>
                    <th class="active" colspan="2">GP 2</th>
                    <th class="active" colspan="2">GP 3</th>
                    <th class="active" rowspan="2">Anniversaires</th>
                </tr>
                <tr>
                    <!-- td from rowspan -->
                    <th class="column">de 10 à 18 pers.</th>
                    <th class="column">de 19 à 38 pers.</th>
                    <th class="column">de 10 à 18 pers.</th>
                    <th class="column">de 19 à 38 pers.</th>
                    <th class="column">de 10 à 18 pers.</th>
                    <th>de 19 à 38 pers.</th>
                    <!-- td from rowspan -->
                </tr>
            </thead>
            <tbody>
                <tr>
                    <td>Tarifs</td>

                    <td>52,00 &#8364;/pers</td>
                    <td>49,00 &#8364;/pers</td>

                    <td>62,00 &#8364;/pers</td>
                    <td>59,00 &#8364;/pers</td>

                    <td>69,00 &#8364;/pers</td>
                    <td>64,00 &#8364;/pers</td>

                    <td>33,00 &#8364;/enfant</td>

                </tr>

                <tr>
                    <td>Temps de roulage</td>
                    <td colspan="2">10 min d'essai/qualif <br>+<br>15 min de course</td>
                    <td colspan="2">10 min d'essai <br>+<br>10 min de qualif <br>+<br>10 min de course</td>
                    <td colspan="2">10 min d'essai<br>+<br>10 min de qualif<br>+<br>15 min de course</td>
                    <td>2 sessions de 10 min</td>
                </tr>

                <tr>
                    <td>Formule comprenant</td>
                    <td colspan="6">Podium</td>
                    <td>Gâteau <br>+<br> bonbons <br>+<br> boissons <br>+<br>diplôme<br>+<br>podium<br>+<br>1 médaille</td>
                </tr>
            </tbody>
        </table>
    </div>
</div>


<div class="row">
    <img src="/assets/img/paiements-acceptes.png" alt="type de paiements acceptés">
    <p>
        <strong>Nous acceptons les espèces en euros, les cartes bancaires CB, VISA et Mastercard ainsi que les chèques
            vacances ANCV.
        </strong>
        <br>
        Malheureusement, nous n'acceptons plus les chèques bancaires.
    </p>
</div>